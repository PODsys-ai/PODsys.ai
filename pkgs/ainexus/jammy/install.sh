...
conf_ip() {
    if [ -f "/iplist.txt" ]; then
        ...
        if [ -n "$IP" ]; then
            ...
            echo "$SN IP is Empty" >>/podsys/log/conf_ip.log
        fi
        ...
        echo "$SN No iplist file" >>/podsys/log/conf_ip.log
    fi
...
install_compute() {
    ...
    if lspci | grep -i nvidia; then
        ...
        if [ "$method" == "http" ]; then
            ...
            ./podsys/nvidia/${NVIDIA_DRIVER} --accept-license --no-questions --no-install-compat32-libs --ui=none --disable-nouveau >>$install_log
        elif [ "$method" == "p2p" ]; then
            ./nvidia/${NVIDIA_DRIVER} --accept-license --no-questions --no-install-compat32-libs --ui=none --disable-nouveau >>$install_log
        else
            wget -q http://$1:8800/workspace/drivers/nvidia.tgz
